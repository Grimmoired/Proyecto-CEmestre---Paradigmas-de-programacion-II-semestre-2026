...
/*
    Busca un curso por código dentro del catálogo (búsqueda lineal, se hace una sola vez por código).
    Retorna el índice dentro de catalog->courses, o -1 si no existe.
*/
...
int fulfillCorequisites(
    ...
    for (int i = 0; i < corequisiteCount; i++) {
        ...
        /* Regla del ciclo de correquisitos:
           si el correquisito es parte del camino actual
           entonces se ignora para evitar ciclos y falsos positivos
           o falsos negativos
        */
        ...
        if (isCodeApproved(corequisites[i], history)) {
            continue; // ya aprobado, esta condición se cumple 
        }

        /* No está aprobado: se puede llevar en paralelo
           si el curso aprueba sus reqs y coreqs
           y se aniade a los ancestros. 
        */
...
void computeEligibility(Catalog *catalog, const StudentHistory *history) {
    for (int i = 0; i < catalog->courseCount; i++) {
        ...
        // El propio curso arranca como el único ancestro de su camino 
...
static void dfsCycles(
    ...
    for (int i = 0; i < course->requisiteCount; i++) {
        ...
        if (neighbor == -1) {
            continue; // dato mal formado, ya se reporta en la carga
        }
        ...
        if (colors[neighbor] == COLOR_WHITE) {
            ...
            /* Ciclo detectado: 'neighbor' es parte del camino actual
               entonces se busca su indice para guardarlo e ir haciendo el camino
            */
            int start = -1;
